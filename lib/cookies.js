...
var pushTo = function(headers, cookie) {
  headers.push(cookie.toHeader())
  if (cookie.value===undefined || (cookies.expires && cookie.expires < 0))
    cookieCache[cookie.name] = "";
  else
...
function Cookies(request, response, keys) {
  ...
  this.keys = keys
  cookieCache = parseCookie(request.headers.cookie);
}
